...
module.exports = {
  ...
  run: async (client, interaction) => {
    if (interaction.user.id !== '654029828193779732') return interaction.reply(nodev)
    ...
    blacklist.findOne({
      ...
      if (data) {
        ...
        interaction.reply({ content: removedblacklist.replace("<usertag>", user.tag)})
      } else {
        interaction.reply({ content: notblacklisted.replace("<usertag>", user.tag)})
      }
    })
  }
}